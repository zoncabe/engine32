...
void physicsHeap_init(PhysicsHeap *h, int32_t size)
{
	h->memory = (PhysicsHeader *)physics_alloc(size);
	psyqo::Kernel::assert(h->memory != NULL, "physics: out of memory");
	h->memory->next = NULL;
	h->memory->prev = NULL;
	h->memory->size = size;
	h->size = size;
...
{
	psyqo::Kernel::assert(memory != NULL, "physics: free of NULL");
	...
	if (prev_block) {
		...
		if (next_block) {
			...
			psyqo::Kernel::assert(h->free_block_count > 0, "physics: heap free list empty");
			psyqo::Kernel::assert(prev_block_idx != ~0, "physics: heap block not found");
			--h->free_block_count;
	...
	if (!merged) {
		...
		if (h->free_block_count == h->free_block_capacity) {
			...
			psyqo::Kernel::assert(h->free_blocks != NULL, "physics: out of memory");
			__builtin_memcpy(h->free_blocks, old_blocks, sizeof(PhysicsFreeBlock) * (size_t)old_cap);
