...
void ContactManifold::setPair(PhysicsShape *a, PhysicsShape *b)
{
	A = a;
	B = b;
...
void ContactConstraint::solveCollision()
{
	manifold.contact_count = 0;

	collision(&manifold, A, B);

	if (manifold.contact_count > 0) {
		if (flags & CONSTRAINT_COLLIDING) {
			flags |= CONSTRAINT_WAS_COLLIDING;
		} else {
			flags |= CONSTRAINT_COLLIDING;
		}
	} else {
		if (flags & CONSTRAINT_COLLIDING) {
			flags &= ~CONSTRAINT_COLLIDING;
